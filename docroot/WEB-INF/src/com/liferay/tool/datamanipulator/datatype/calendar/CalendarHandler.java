...
public class CalendarHandler extends AbstractPortletHandler
	...
	public void startGenerate(RequestProcessor requestProcessor)
		...
		int eventCount = requestProcessor.getCount(
			EntryTypeKeys.GENERAL_CALENDAR_EVENT);

		int eventUpdate = requestProcessor.getUpdateLevel(
			EntryTypeKeys.GENERAL_CALENDAR_EVENT);

		EntryTypeReader eventEntryType = EntryReaderUtil.getEntryType(
			EntryTypeKeys.GENERAL_CALENDAR_EVENT);

		BaseEntry eventEntry = new BaseEntry(eventEntryType);

		EntryHandlerModel eventHandler = new CalendarEventHandler(
			eventCount, eventUpdate, 0, 0, eventEntry, null, requestProcessor);

		eventHandler.generateEntries((long)0);
	}
